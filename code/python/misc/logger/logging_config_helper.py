...
from .logger import LogLevel, LoggerUtility, correlation_id_var
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"Silent fallback to defaults disabled to prevent loss of observability."
            ) from e
...
class AsyncLogProcessor:
    ...
    def _worker(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                module_name, level, message, args, kwargs, corr_id = log_record

                # Restore correlation_id in worker thread context
    ...
    def enqueue_log(self, module_name: str, level: str, message: str, *args, **kwargs):
        ...
        if not self.shutdown_event.is_set():
            # Capture correlation_id from caller's context before crossing thread boundary
            corr_id = correlation_id_var.get()
            log_record = (module_name, level, message, args, kwargs, corr_id)
...
class LazyLogger:
    ...
    def exception(self, message: str, **kwargs):
        ...
        # Capture exc_info at the call site — sys.exc_info() returns (None, None, None)
        # in the background worker thread where there is no active exception.
