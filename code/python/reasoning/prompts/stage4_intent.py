...
- special_elements: 從訊息中抽出「特殊格式元素」訴求（typed list, type ∈
  ...
  * target_chapter：填**章節名稱原文**（例「結果與討論」「結論」「前言」）為主。
    - user 若列了章名 → 填章名原文。
    - user **只用序數**指涉（例「第四章要有表格」）且沒列章名 → 保留序數字串「第四章」即可
      （抽取階段還沒有最終章名；系統之後會用暫定章名判斷，對得到就放、對不到就回頭問你）。
    - user **明說「每章 / 全部 / 全章都要」** → 空字串 ""（= 全章注入）。
    - user **「加表格」未說在哪章 / 「找個地方放」** → 填**原文非空**（如「加表格」「找個地方」），
      **不要填空字串**（空字串專指「明說全章」；未指定要讓系統回頭問你）。
...
只回傳 JSON，不要其他文字。"""
    ...
    def build_response_classifier_prompt(
        ...
        chapter_names=None,  # R2：暫定章名清單（供 LLM 判 target 語意對應）
    ) -> str:
        ...
        _names = chapter_names or []
        chapter_block = (
            "目前規劃的章節（供你判斷 special_element 的 target 放哪章）：\n"
            + ("、".join(f"{i+1}）{n}" for i, n in enumerate(_names)) if _names
               else "（章節尚未定案）")
            + "\n"
        )
...
### special_element target 語意判斷（R2，OQ-6 靈魂）
對每個 special_element，除了填 target_chapter 外，**再判它語意上對應上方哪一章**：
- 若 user 用**語意指涉**（「放在講政策的那章」「加在比較各國的地方」「結果那段」）→ 對照上方章節清單，
  判它語意最接近哪一章，填 resolved_chapter_title=該章名、resolution_confidence="clear"。
  （你很聰明，「講政策」≈「結果與討論」、「比較各國」≈「國外案例」這種對應要判得出來。）
- 若 user 用**明確章名 / 序數**且你能對到唯一一章 → 一樣填 resolved_chapter_title + "clear"。
- 若**判不準 / 多章都可能 / 完全對不到**任何現有章 → resolved_chapter_title=""、resolution_confidence="uncertain"。
- **★ target_chapter 空字串語意（B2，嚴格）**：`target_chapter=""` **只**用於 user **明說「每章都要 / 全部章節 /
  全章」**（= 明確全章注入，此時 resolution_confidence 也留 null，系統直接全章注入不問）。
  user **「找個適合的地方」/ 沒指涉哪章** → target_chapter **填原文（如「找個適合的地方」，非空）**
  + resolution_confidence="uncertain"（讓系統問 user 放哪章，不要 silent 塞全章）。
- **校準錨點（別過度問也別亂猜）**：「猜錯代價 > 問一句成本」才標 uncertain；語意明確對得到就標 clear
  由系統發一次輕量確認。exact 章名 / 唯一解這種明擺著的，系統會在你之前用 code 直接處理，你專注語意判斷。
...
case 3b — 章節清單有「前言/國內案例/國外案例/結果與討論/結論」、user reply「表格放在講政策的那章」：
  → {{"action":"add_special_element", "format_content":{{"special_elements":[
      {{"type":"table","target_chapter":"講政策的那章",
        "resolved_chapter_title":"結果與討論","resolution_confidence":"clear","description":""}}
    ]}}}}

case 3c — 同章節清單、user reply「找個適合的地方放個表格」（沒指涉哪章，B2：target 非空 + uncertain）：
  → {{"action":"add_special_element", "format_content":{{"special_elements":[
      {{"type":"table","target_chapter":"找個適合的地方",
        "resolved_chapter_title":"","resolution_confidence":"uncertain","description":""}}
    ]}}}}

case 3d — 同章節清單、user reply「每一章都放一個表格」（明說全章，B2：target 空 = 全章注入不問）：
  → {{"action":"add_special_element", "format_content":{{"special_elements":[
      {{"type":"table","target_chapter":"",
        "resolved_chapter_title":"","resolution_confidence":null,"description":""}}
    ]}}}}
