...
def _build_relevance_prompt(query: str, digest: str) -> str:
    ...
    2026-08-12（UX 回報 row7「問再生能源卻回再生水」）：補「主題同一性」條——原本的
    負面表列只涵蓋**具名人物**張冠李戴，概念/主題類查詢無對應條款，於是「共用字詞、
    同屬上位領域」的來源被「間接相關要保留」+「不確定就保留」兩條預設一路放行
    （實測 3/3 全保留）。新條與人物條同級並明寫為前者的例外，避免被抵銷。
    機械防線：tests/test_relevance_gate_core.py::TestSubjectIdentityRule。
    """
    return (
        ...
        "- 查詢主體是主題／概念／政策／技術／產業時：來源談的是**另一個主題**，"
        "即使名稱共用部分字詞、或同屬更上位的領域，也屬完全不相關——"
        "共用字詞不等於同一個主題。判斷依據是來源整篇在講什麼，"
        "不是查詢的字詞有沒有出現在來源裡。（此條與上一條同級，是"
        "「間接相關要保留」的例外，不受其約束。）\n"
