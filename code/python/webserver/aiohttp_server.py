...
class AioHTTPServer:
    ...
    async def _on_startup(self, app: web.Application):
        ...
        # Initialize user data database (idempotent — creates any missing tables)
        try:
            from core.user_data_db import get_user_data_db
            user_db = get_user_data_db()
            await user_db.initialize()
            # Clean up zombie upload states from previous crashes/restarts
            await user_db.execute(
                "UPDATE user_sources SET status = 'failed', error_message = 'Server restarted during processing' "
                "WHERE status IN ('processing', 'uploading')"
            )
            logger.info("User data DB initialized + zombie states cleaned")
        except Exception as e:
            logger.error(f"User data DB initialization failed: {e}", exc_info=True)
    ...
    async def _on_cleanup(self, app: web.Application):
        ...
        # Close auth DB connection pool (timeout to avoid blocking shutdown)
        try:
            from auth.auth_db import AuthDB
            auth_db = AuthDB.get_instance()
            await asyncio.wait_for(auth_db.close(), timeout=3.0)
        except Exception:
            pass

        # Close analytics DB connection pool (timeout to avoid blocking shutdown)
        try:
            from core.analytics_db import AnalyticsDB
            analytics_db = AnalyticsDB.get_instance()
            await asyncio.wait_for(analytics_db.close(), timeout=3.0)
        except Exception:
            pass
    ...
    async def start(self):
        ...
        # Keep server running with proper CTRL+C handling
        ...
        # Windows: select() is not interrupted by SIGINT, so signal handlers
        # don't fire until select() returns. This periodic wakeup ensures
        # CTRL+C is processed within 0.5s.
        wakeup_task = None
        if sys.platform == 'win32':
            async def _wakeup():
                while True:
    ...
    async def stop(self):
        """Stop the server gracefully (idempotent — safe to call multiple times)"""
        if self.site:
