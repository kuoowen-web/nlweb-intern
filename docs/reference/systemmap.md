...
**🔄 Major Upgrade 升級方向**（詳見 `docs/in progress/plans/major-upgrade-plan.md`）：

| 升級概念 | 狀態 | 說明 |
|----------|------|------|
| **Composable Pipeline Refactor** | 🔄 執行中 | `run_research()` 850 行單體 → ResearchState dataclass + 4 個 composable phases（filter_and_prepare → actor_critic_loop → writer → format_result） |
| **Association Layer（B→A→B' Loop）** | 📋 計畫中 | 在 Retrieval 前建立 Context Map (master B)，衍生 Search Plan (A)，execute 後 refine B。Session-wide master B。現有 Gap Detection 是 proto 版本（reactive），升級為 proactive |
| **Critic Agent 擴充** | 📋 計畫中 | 新增 `review_consistency()` — 檢查 research 方向是否 align 初始架構（master B drift 偵測）。新 output channel：讀豹對話轉折訊息。不新建組件 |
| **Propose-Verify Pattern** | 📋 計畫中 | LLM knowledge = falsifiable hypothesis → Google Search / HTTP Scrape verify → 只有 confirmed 的進 candidate list。跟現有 Hallucination Guard + CoV 形成三層事實保護 |
| **Mini-Reasoning per Step** | 📋 計畫中 | 每步驟是一個 mini reasoning module（internal Actor-Critic）。Hierarchical: MetaOrchestrator → StepOrchestrator → 現有 DeepResearchOrchestrator（reuse） |
| **Association Agent** | 📋 計畫中 | B→A→B' loop 引擎，作為新 agent 加入 Reasoning 模組 |
...
**🔄 Major Upgrade 升級方向**（詳見 `docs/in progress/plans/major-upgrade-plan.md` §5.4, §6.6）：
...
> 詳細計畫：`docs/in progress/plans/major-upgrade-plan.md`
...
*更新：2026-05-25（Frontend Modular Refactor v4.0 Path A++ 完成，M5 Frontend 模組架構段全面更新）*
