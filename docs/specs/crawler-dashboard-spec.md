...
> **⚠️ Vectors 卡片已下線（2026-07-10 reconcile）**：原「Vectors | `qdrant.vectors_count`」卡片已移除——`/api/indexing/stats`（`dashboard_api.py` `get_stats`）現只回 `registry` + `timestamp`，`_get_qdrant_stats` 已刪，response 不再含 `qdrant` key（Qdrant 於 2026-06-22 廢除）。前端 `indexing-dashboard.html` 仍以 `data.qdrant?.vectors_count || 0` optional-chaining 讀取，故該欄現恆顯示 0（待前端一併清理）。
...
> **Reconcile 2026-07-10**：先前 response 範例末尾的 `"qdrant": {"vectors_count": 45000}` 已移除——`get_stats` 現只回 `registry` + `timestamp`（`_get_qdrant_stats` 已刪，Qdrant 2026-06-22 廢除）。
