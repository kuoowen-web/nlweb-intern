...
├── scripts\                # 3 個 hook script（settings.json 以絕對路徑呼叫）
│   ├── py-compile-check.py     # PostToolUse Edit|Write：.py 編輯後語法檢查
│   ├── intern-sync-reminder.py # PostToolUse Bash：commit 動到技術碼時提醒同步 intern repo
│   └── cleanup-reminder.py     # Stop：session 結束提醒清理臨時檔
├── hooks\                  # dispatcher 化 hook（2026-07 上線）
│   ├── pre_bash_dispatcher.py       # PreToolUse Bash
│   ├── pre_agent_dispatcher.py      # PreToolUse Task|Agent
│   ├── post_bash_dispatcher.py      # PostToolUse Bash
│   ├── post_edit_write_dispatcher.py # PostToolUse Edit|Write
│   ├── read_coverage_gate.py        # Stop + SubagentStop（H16 read-coverage gate）
│   └── read_coverage_parser.py      # read_coverage_gate 的 parser 輔助
├── memory\
...
現況 `settings.json` 共掛載 **4 個事件、9 個 hook 呼叫**（2026-07 已 dispatcher 化，並上線 Stop / SubagentStop）：

| Hook 事件 | matcher | 觸發時機 | 功能（依 settings.json 順序） |
|-----------|---------|----------|------|
| **PreToolUse** | `Grep` | 嘗試使用 Grep 前 | exit 2 阻擋，強制改用 SQLite 索引搜尋 |
| **PreToolUse** | `Bash` | 每次 Bash 前 | `hooks/pre_bash_dispatcher.py`（bash 前置分派） |
| **PreToolUse** | `Task\|Agent` | 派 subagent 前 | `hooks/pre_agent_dispatcher.py`（agent 前置分派） |
| **PostToolUse** | `Edit\|Write` | 編輯/寫入檔案後 | ①`scripts/py-compile-check.py`（.py 語法檢查）②`hooks/post_edit_write_dispatcher.py` |
| **PostToolUse** | `Bash` | 每次 Bash 後 | ①`scripts/intern-sync-reminder.py`（commit 動技術碼提醒，非阻塞）②`hooks/post_bash_dispatcher.py` |
| **Stop** | （無 matcher，全域） | 主 session 停止時 | ①`scripts/cleanup-reminder.py`（清理臨時檔提醒）②`hooks/read_coverage_gate.py`（**H16 read-coverage gate**） |
| **SubagentStop** | （無 matcher，全域） | subagent 停止時 | `hooks/read_coverage_gate.py`（同 H16 gate，對 subagent 生效） |

另有 `enabledPlugins`：`security-guidance@claude-plugins-official`。

> **Stop / SubagentStop hook 已上線（H16 read-coverage gate）**：早期文件曾論述「不可用 Stop hook 因會無限迴圈（hook 輸出 → Claude 回應 → 再次觸發 Stop → 循環）」——此論述**已過時作廢**。現況 Stop hook 設計為**非對話性輸出 / 有終止條件**，不會遞迴觸發：`read_coverage_gate.py` 是 read-coverage 稽核（MUST_READ_FULL 機制：被指示「必讀」的檔案若只讀頭尾/關鍵字未達覆蓋率，session 結束時 gate 攔截），`cleanup-reminder.py` 是一次性清理提醒。兩者都不會產生「觸發 Claude 再回應 → 再觸發 Stop」的迴圈。
>
> **H16 read-coverage gate（`hooks/read_coverage_gate.py` + `read_coverage_parser.py`）**：對「MUST_READ_FULL」標記的必讀檔案做覆蓋率稽核，Stop / SubagentStop 時檢查是否全讀（防「只讀關鍵字段落就宣稱讀過」）。對應 `docs/in progress/plans/h16-read-coverage-hook-plan.md`。
>
...
**Windows 注意事項**：
- `$CLAUDE_PROJECT_DIR` 在 Windows CMD 不展開。現況所有 hook（`scripts/*.py` + `hooks/*.py`）**直接寫絕對路徑**（`python C:/users/user/nlweb/.claude/scripts/...` 或 `.../hooks/...`）以求穩定；若要可攜，改相對路徑，勿用 `$CLAUDE_PROJECT_DIR`。
- 不可使用 `|| true`（`true` 不是 Windows CMD 指令）
- 不可使用 `2>/dev/null` → 移除或改用 `2>NUL`
- `echo` 不需要單引號（CMD 會把引號當作輸出內容）
- **Stop / SubagentStop hook 可用**（H16 已上線）——關鍵是 hook 輸出必須**非對話性 / 有終止條件**，不可產生「觸發 Claude 回應 → 再觸發 Stop」的遞迴；`read_coverage_gate.py` / `cleanup-reminder.py` 皆符合此條件。舊「不可用 Stop hook」的結論已作廢。
...
| 協作回饋 | `feedback_*` | `feedback_e2e_testing.md`、`feedback_no_auto_push.md` |
...
| Ranking | `docs/specs/xgboost-spec.md`、`docs/specs/mmr-spec.md`（🪦 bm25-spec 已歸檔 `docs/archive/specs/`，現行全文匹配見 systemmap M2/M3）|
...
| **PreToolUse** | `Bash` | `hooks/pre_bash_dispatcher.py` |
| **PreToolUse** | `Task\|Agent` | `hooks/pre_agent_dispatcher.py` |
| **PostToolUse** | `Edit\|Write` | `scripts/py-compile-check.py` + `hooks/post_edit_write_dispatcher.py` |
| **PostToolUse** | `Bash` | `scripts/intern-sync-reminder.py`（非阻塞）+ `hooks/post_bash_dispatcher.py` |
| **Stop** | （全域） | `scripts/cleanup-reminder.py` + `hooks/read_coverage_gate.py`（H16） |
| **SubagentStop** | （全域） | `hooks/read_coverage_gate.py`（H16） |

另有 `enabledPlugins`：`security-guidance@claude-plugins-official`。

> **Stop / SubagentStop 已上線（H16 read-coverage gate）**；無 PreCompact、無 TodoWrite milestone（Smart Compacting 已退役）。
> hook 路徑現況為絕對路徑（`C:/users/user/nlweb/.claude/scripts/...` 或 `.../hooks/...`）。dispatcher 化後 `.claude/hooks/` 目錄承載 pre/post bash + agent + edit/write + read_coverage_gate。
...
| 問題 | 症狀 | 原因 | 修正 |
|------|------|------|------|
| **Stop hook 無限迴圈**（早期，**已解**） | Session 結束後 Claude 不斷回應，無法停止 | 早期 Stop hook **輸出對話性內容 / 無終止條件** → Claude 回應 → 再次觸發 Stop | 早期做法：移除所有 Stop hooks。**現況（2026-07）**：Stop / SubagentStop 已重新上線（H16 read-coverage gate + cleanup-reminder），關鍵是 hook 輸出**非對話性 + 有終止條件**，不再遞迴。詳見上方 Hooks 章節 |
| **Rate limit 無限迴圈**（早期，**已解**） | 遇到 rate limit 時 hook 持續觸發 | 同上機制在 rate limit 狀態放大 | 同上——改為非對話性 / 有終止條件的 Stop hook 後不再發生 |
...
- **Stop hook** → 早期因對話性輸出造成無限迴圈；**現已上線**（H16 read-coverage gate + cleanup-reminder），只要輸出非對話性 / 有終止條件即安全（詳見 Hooks 章節）
...
*2026-07-10：hook 清單追現況 — settings.json 已 dispatcher 化 + Stop/SubagentStop 上線（H16 read-coverage gate）。更新為 4 事件 9 呼叫；作廢「無 Stop hook / 不可用 Stop hook 因無限迴圈」過時論述（改為「輸出非對話性 + 有終止條件即安全」）；補 `.claude/hooks/` 目錄（pre/post bash·agent·edit-write dispatcher + read_coverage_gate/parser）；`.claude/scripts/` 更正為 3 檔（py-compile-check / intern-sync-reminder / cleanup-reminder）。commands 13 個不變*
