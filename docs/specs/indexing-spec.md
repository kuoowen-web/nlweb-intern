...
> **🪦 .indexing_done / .pg_indexing_done 注意（歷史）**：~~舊 `.indexing_done` 是 Qdrant 時代殘留~~ → 已完成（2026-03）。其後曾由 `pg_batch.py` + `.pg_indexing_done` 追蹤桌機 PG indexing 進度（checkpoint 為 `<tsv>.pg_checkpoint.json`）——**pg_batch.py 已隨路徑 A 刪除批移除（2026-07-16，回溯 `1d150e49`）**，兩類 checkpoint 檔為桌機殘留 runtime 檔、不再有任何寫入者。現行進度追蹤分兩層：orchestrator/VPS 權威 done-set `/data/indexed_tsvs.txt`（B 路增量鏈，見 `scripts/indexing_orchestrator.sh` 與 incremental-backfill plan）；`bulk_load.py` 本機斷點另為 `.bulk_load_done`（見 bulk-load-spec）。
...
實際上是多條寫入路徑併存（終點同為 PostgreSQL，中途模組不同）：

```
路徑 B（prod 主力 — 全量 + 增量 backfill 皆走此路）：
  TSV → cloud_embed.py（GPU VM；行內 parse/quality/chunk ＋ Qwen3-4B INT8 embed）
      → {name}.jsonl + .npy → bulk_load.py（np.load mmap + INSERT ON CONFLICT）
      → VPS PostgreSQL（articles + chunks）
  ※ 2026-07-11 起增量鏈由 orchestrator/launcher 驅動、無 GCS 中轉直送 VPS；
    2026-03 歷史全量批次與 weekly_indexing.sh 走 GCS 中轉。

路徑 C（私人文件上傳）：
  檔案 → UserDataProcessor → core/chunking.py（tiktoken 500tok）
      → core/embedding.py（API）→ user_document_chunks（詳見 private-docs-spec.md）

路徑 A（桌機本地 dev DB 批次）：🪦 已移除 2026-07-16（D-2026-07-16 刪除不修復；
        回溯 git ref 1d150e49）。原鏈：TSV → pg_batch.py（IngestionEngine →
        QualityGate → ChunkingEngine → PostgreSQLUploader）→ 桌機 localhost PG。
```

> 舊圖「TSV → Ingestion → Quality Gate → Chunking → Dual Storage（Map + Vault）」為
> Qdrant 時代單一管線描述，已不反映現實（Dual Storage 見檔內 🪦 歷史章節）。
> 注意：路徑 B 的 parse/quality/chunk 為 cloud_embed.py 行內實作；已刪除的路徑 A
> 檢查面較全（比較細節見 backlog 2026-07-16-d，回溯 ref 1d150e49）。
...
Crawler 輸出的 TSV 直接作為 Indexing Module 的輸入（現行主路徑 B）：

```
[Crawler]                  [Indexing — 現行主路徑 B]
    ↓
  TSV 檔案  ─────────→  cloud_embed.py（GPU VM：行內 parse/quality/chunk + embed）
(url<TAB>JSON-LD)              ↓
                        {name}.jsonl + .npy
                               ↓
                        bulk_load.py → VPS PostgreSQL
```

> 舊圖（IngestionEngine → QualityGate → ChunkingEngine → Dual Storage）描述的是
> 桌機路徑 A + Qdrant 時代儲存，路徑 A 🪦 已移除 2026-07-16（見「架構流程」段註記）。
...
> 🪦 原「TSV 送入 `python -m indexing.pipeline`」教學已隨路徑 A 刪除批移除 2026-07-16（回溯 ref 1d150e49）。現行驗證走 B 路：cloud_embed canary + bulk_load（見 `bulk-load-spec.md` 與 indexing-incremental-backfill-plan）。
...
# Step 2: 將 TSV 送入現行 B 路（GPU VM 上跑 cloud_embed，產出由 bulk_load 灌 VPS PG）
#   python indexing/cloud_embed.py <tsv> && python indexing/bulk_load.py <jsonl> <npy>
#   （詳細參數與 canary 流程見 bulk-load-spec.md）
```
...
> 🪦 檔案已刪除 2026-07-16（D-2026-07-16 路徑 A 刪除批；回溯 git ref 1d150e49）

管理新聞來源的可信度分級。

> **消歧義（2026-07-10 記；🪦 2026-07-16 更新）**：本節的 SourceTier（Tier 1-4）**原為 indexing 層 `source_manager.py` 的機制、由 `pipeline.py` 使用**，僅作品質標記——**兩檔均已隨路徑 A 刪除批移除（2026-07-16，回溯 `1d150e49`），此機制僅存 git 史**。它與 2026-06 已徹底廢除的 **reasoning 層來源分級**（Tier 1-5 權威分級 + 「未經證實」警語，現為 pass-through）是兩套不同機制，勿混淆。
...
> 🪦 **歷史紀錄（Qdrant 路徑 2026-06-22 廢除；pipeline.py 本體 2026-07-16 隨路徑 A 移除，回溯 `1d150e49`）**：`pipeline.py` 曾以 Vault/SQLite pipeline 形式殘存（其 `--upload`/`--reconcile` 能力已隨 `qdrant_uploader.py` 移除而作廢），桌機批次入口 `pg_batch.py` 亦同批移除。**現行唯一主路徑**為 `cloud_embed.py` + `bulk_load.py`（B 路），見「儲存與 Embedding：單一現役路徑」章。
...
> 🪦 `config/config_indexing.yaml` 已隨路徑 A 刪除批移除 2026-07-16（D-2026-07-16；回溯 git ref 1d150e49），本節為歷史紀錄。現行 B 路參數為 `cloud_embed.py` 行內常數。
...
> 🪦 本節描述的路徑 A / Qdrant 時代模組鏈已刪除 2026-07-16（D-2026-07-16 路徑 A 刪除批；回溯 git ref 1d150e49），保留作歷史。現行資料流見「架構流程」段路徑 B。
...
> 🪦 下方範例使用的 vault_helpers（get_full_text_for_chunk）已隨路徑 A 刪除批移除 2026-07-16（回溯 git ref 1d150e49），僅存歷史；現行原文取得直接查 PostgreSQL `articles.content`。

```python
# 取得原文（使用 chunk_id，非 url）— 🪦 歷史範例
...
```
code/python/indexing/     # 終態（2026-07-16 路徑 A 刪除批後）：5 檔
├── __init__.py           # 註解塊-only（不 re-export 任何符號）
├── cloud_embed.py        # 【主路徑 B】GCP L4 VM：chunking + Qwen3-4B → .jsonl/.npy
├── bulk_load.py          # 【主路徑 B】.jsonl+.npy bulk insert 到 VPS PG（見 bulk-load-spec.md）
├── dashboard_server.py   # Crawler 監控 dashboard（port 8001）
├── dashboard_api.py      # Dashboard API routes
│
│   # 以下為已刪除檔案（🪦 回溯 git ref 1d150e49）
├── source_manager.py     # 來源分級 — 🪦 檔案已刪（2026-07-16）
├── ingestion_engine.py   # TSV 解析 — 🪦 檔案已刪（2026-07-16）
├── quality_gate.py       # 品質驗證 — 🪦 檔案已刪（2026-07-16）
├── chunking_engine.py    # 分塊引擎 — 🪦 檔案已刪（2026-07-16）
├── postgresql_uploader.py # 路徑 A PG 上傳 + Qwen3-4B INT8 embedding + GPU thermal — 🪦 檔案已刪（2026-07-16）
├── pg_batch.py           # 路徑 A PG 批次 indexer（PGCheckpoint 續傳）— 🪦 檔案已刪（2026-07-16）
├── dual_storage.py       # 【Qdrant 路徑】雙層儲存（MapPayload + VaultStorage）— 🪦 檔案已刪（2026-07-16）
...
├── rollback_manager.py   # 回滾管理 — 🪦 檔案已刪（2026-07-16）
├── pipeline.py           # 【Qdrant 路徑】Vault pipeline + CLI — 🪦 檔案已刪（2026-07-16）
├── vault_helpers.py      # 【Qdrant 路徑】Async helpers — 🪦 檔案已刪（2026-07-16）
└── poc_*.py              # POC 驗證腳本 ×5 — 🪦 檔案已刪（2026-07-16）

config/
└── config_indexing.yaml  # 配置檔 — 🪦 檔案已刪（2026-07-16）

data/
├── vault/
│   └── full_texts.db     # Vault 資料庫（🪦 殘留 runtime 檔，無寫入者）
└── indexing/
    ├── buffer.jsonl      # 品質不合格緩衝（🪦 殘留 runtime 檔，無寫入者）
    └── migrations.db     # 遷移記錄（🪦 殘留 runtime 檔，無寫入者）
```
...
| 上傳模組 | `cloud_embed.py` + `bulk_load.py` | `qdrant_uploader.py` + `embedding.py` |
| 批次驅動 | `pg_batch.py` — 🪦 已刪 2026-07-16（路徑 A 刪除批） | `pipeline.py --upload` |

> **同一 chunk 在兩路嵌入的模型不同**（Qwen3-4B vs OpenAI/bge-m3），向量空間不可互通。這是已知狀態，是否收斂為另案。

### PostgreSQL Schema（主路徑）

schema 現行寫入者為 `bulk_load.py`（🪦 歷史上由 postgresql_uploader.py 建立並取代 `qdrant_uploader.py` + `VaultStorage`，該檔已刪 2026-07-16）；DDL 見 `infra/init.sql`。兩張表：
...
- **去重兩層**：URL 層 `ON CONFLICT (url)`；同篇不同 URL 由 `(title, source)` title dedup（🪦 歷史實作為 `postgresql_uploader.py` `_insert_article`，該檔已刪 2026-07-16）。
...
> 🪦 pg_batch.py / postgresql_uploader.py 已刪除 2026-07-16，本段為歷史紀錄（回溯 ref 1d150e49）
