...
5. **大量節點效能**：節點數超過 50 時，sector 角度過密，label 容易重疊（前端 layout 限制）。無自動 label 避讓機制。
   - **門檻不一致註記**：本處前端 layout 的 label 重疊起點為 **50**；而後端 LR Track D merge 後在 `len(entities) > 100` 才記 `logger.warning`（「KG render layout may break」，且 warning 訊息反向引用本 §11）。兩數字語意不同（50 = label 重疊起點；100 = 後端認定 layout 可能崩潰的告警門檻），目前刻意未對齊，讀者勿混為一談。
