...
- **pending 恢復意圖判定**：user 對澄清問句的回答意圖**交 LLM 判**（`_classify_pending_special_element_reply`：
  confirm/change_chapter/cancel/unclear），**禁 substring/regex 解析 user 自然語言回答**（中文序數/複合否定規則式必漏）。
...
| **Real Persona E2E** | `mock_bab=false` + 真實 retrieval + 真實 BAB + 真實 persona reply | 內部 persona 全程真實 | 高（~$5） | ✅ release 前 ≥ 1 次 |
...
預留 vendor 記者 persona / 其他 vendor 訪談 persona / B2B 客戶 persona。
