...
> **最後 code re-sync**：2026-06-19（`main`）
...
4. _run_publish_gate(section, ...)                        ← F1 critic + F3 CoV-lite（§6.8）
  ...
  ↓
5. _maybe_truncate_chapter(section, target_word_count)   ← 字數 post-process 硬切（§4.7.9）
   └─ 實際字數 > target × _WORD_OVERSHOOT_RATIO → 硬切到 target 附近 + 切句界 + 補省略號 + 發旁白
  ↓
...
#### 4.7.9 章節字數 post-process 硬切（bug 2026-06-20）

**背景**：字數對齊原本走軟約束（outline planner 分配 `target_word_count` budget §4.7.5 + writer prompt ±15% + 超標時發「內容照常保留」透明化旁白）。**prod 證實軟約束壓不住字數**（target=800 actual=2258）。CEO/CTO 拍板改 **post-process truncate**（硬切到 target 附近），由 `_maybe_truncate_chapter` 在 §4.7.7 守門管線**第 5 步**（publish gate 之後、citation render 之前）執行。

**契約**：
- **閾值**：與舊超標旁白同閾值 `_WORD_OVERSHOOT_RATIO`（超標 30% 才切，正常波動不切）。
- **切點**：`_truncate_chapter_to_target` 在 citation render 之前切（content 仍是 `{cite:N}`）→ 句末標點落在 placeholder 之外，切中文句界（。！？；）天然不切到半個 placeholder；另加防禦（殘留半個 `{cite` 退回上一個完整 placeholder）。句界離 limit 太遠（< target×`_TRUNCATE_MIN_BOUNDARY_RATIO`=0.6）則寧可硬切。
- **字數度量**：用 `_count_chapter_words`（剝 `{cite:N}` 後字元數），與 target 對齊；limit 完全參數化 = 該章 `target_word_count`，無 magic default（target≤0 → no-op）。
- **no silent fail**：節略一律補省略號 `…` + 發 `lr_copy.chapter_word_truncated_narration`「已自動節略到規劃字數附近」誠實旁白（與舊「超標但照常保留」旁白語意區隔）。
- **status gate**：僅 `status=="drafted"` 才切（被 block / critic_rejected / guard_failed 的替換文字數無意義，且切替換文會破壞 sentinel）。
- **不觸發 auto-rewrite**：truncate 直接 mutate `section_output.section_content`，**不**回呼 `writer.compose_section`（避免多輪重寫迴圈）。

**與 critic WARN explanation 區隔（§6.8 / 行內 §「WARN explanation 完整輸出契約」）**：那個 scope 100 字、契約是**不得截斷**（攔腰砍給半句 warning，曾因 `_WARN_EXPLANATION_MAX` 副產物踩雷）；章節是 800-2000 字級別、契約是**要硬切**。兩者**不可複用** truncate helper。

**測試**：`tests/unit/reasoning/test_lr_chapter_truncate.py`（切句界 / 不破壞 citation / 省略號 / 字數收斂 / status gate / 旁白文案）。
...
**後端配套**：Stage 6 在 emit `final_result` 前把 `state.final_report_markdown` 設為整份報告（H1 研究問題 + sections + references + KG markdown），隨 checkpoint boundary 落 `live_research_state` JSONB，前端主路徑直讀，與 export 逐字一致。
...
預留其他 vendor 訪談 persona / B2B 客戶 persona。
...
| 4 | recollect 缺專屬按鈕 | 中 | 「資料不足 → 補搜（recollect）」只能 user 打字觸發，無按鈕（backward-nav 按鈕走 `reset_to_stage` 不補 evidence）。已補後端引導文案，前端按鈕待做。|
...
| 2026-06-23 | **SSE 522 連線釋放治本（§7.3.1）**：detach-aware await（斷線釋 fd、task 不 cancel）+ route finish_response + 路 A slot 綁 task 終態（修 Gemini C1 並行雙寫）+ 移除冗餘 trailing save。已 deploy，待真機 fd/429 驗 |

> 完整 commit 群與踩坑 lesson 見 `memory/lessons-live-research.md`。

---

*最後 code re-sync：2026-06-23（`main`，新增 §7.3.1 連線層 fd 釋放）。本文已從歷史/未實作設計脂肪瘦身為純契約文件；新人導覽見 `docs/reference/lr-onboarding.md`。*
