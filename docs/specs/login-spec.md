...
> **背景**：B2B 共用電腦切帳號時，`localStorage` 是 origin-scoped 不是 user-scoped — admin 登出後同瀏覽器 member 登入仍看到 admin sessions。修法見 commit `24d39f4` + D-2026-05-01 + `memory/lessons-auth.md`「Logout 也清 user-scoped localStorage」段落。

**`AuthManager.USER_SCOPED_KEYS`**（`static/news-search.js:7-14`）— 6 個 user-scoped localStorage keys：
...
**`_clearUserScopedStorageIfUserChanged(newUserId)`**（`static/news-search.js:46-67`）— 偵測 user 變化才清。同 user 重登保留 cache（D-2026-03-13「localStorage 為主」匿名 / 離線情境的紀律仍有效）。

**`AuthManager.logout()`**（`static/news-search.js:157-165`）— 呼叫後端 `POST /api/auth/logout` 撤銷 refresh token，再呼叫 `_handleAuthFailure()` 統一清理。

**`_handleAuthFailure()` 中 USER_SCOPED_KEYS 清除**（`static/news-search.js:213-220`）— logout 場景**無條件清** 6 個 keys，不檢查 user_id 是否變化。
...
**`AuthManager._handleAuthFailure()` 完整流程**（`static/news-search.js:200-241`）：
...
**`UserStateSync` module 三函式**（`static/news-search.js`）：
...
**`assertUserIdentity(cached, fresh)` helper**（`static/news-search.js`）— mismatch 拋 `UserStateSyncError`，caller 必須 `try/catch` 後 trigger A 整套 reset。
