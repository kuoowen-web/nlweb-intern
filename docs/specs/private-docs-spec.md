...
|------|-------------|----------------------|
| chunk_size | 500 tokens | 170 字/chunk（cloud_embed.py 行內實作；🪦 原 chunking_engine.py 已刪 2026-07-16，回溯 `1d150e49`） |
...
呼叫 `core/embedding.get_embedding(text, timeout=60)`，由 `config_embedding.yaml` 決定 provider（現行 production 為 `openrouter`，fallback `deepinfra`，均為 API 呼叫；本機 `qwen3` provider 為 dormant 備援，僅在 config 切換或 dev query param 覆寫時啟用——2026-07-16 對 config 親驗校正，舊述「production 為 qwen3」已過時）。
...
`user_document_chunks` 依 `user_id` / `org_id` 的 **btree 過濾索引**（`idx_udc_user_id`、`idx_udc_user_org`）確保 DB-level 快速過濾，搭配搜尋 SQL 強制 `WHERE user_id = %s`，向量比對不會跨使用者掃描。（註：這些是 btree 索引，**不是向量索引**——本表無任何向量索引，見上方「索引」段與已知限制 3。）
