...
│   ├── loop_engine.py       # BABLoopEngine（B → A → B' 可複用迴圈引擎）
│   ├── stage_state.py       # LiveResearchStageState dataclass（跨 request 持久化）
│   └── fixtures/
│       └── real_energy_policy_state.json   # mock_bab fixture
├── agents/
│   ├── base.py              # Agent 基礎類別
│   ├── analyst.py           # 分析 Agent
...
| `fixtures/real_energy_policy_state.json` | `live_research_mock_bab` flag 啟用時的 fixture ContextMap，跳過 Stage 1+2 LLM 呼叫 | — |
...
UNKNOWN_SOURCE_CONFIG = {
    "default_tier": 4,
    "unknown_source_action": "include_with_warning"
}
```
...
| Phase 3 output | `final_report`, `plan`, `hallucination_corrected` |
