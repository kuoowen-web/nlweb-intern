...
| Cross-user 隔離 | `UserStateSync.fullReset`（`core/state-sync.js`）+ `AuthManager._handleAuthFailure`（`core/auth-manager.js`）— 舊 `_clearUserScopedStorageIfUserChanged` 已 Task 13 移除 |
...
> **⚠️ 2026-07-14 修正（`ff989303`，74 天 silent data loss regression）**：本清單原缺「**一般搜尋結果完成**」點——2026-05-01 dirty-gate 引入時只標 8 點（漏此點），導致搜尋結果 save 被 gate 無聲吞掉、結果從此不落庫（潛伏 74 天，DB 症狀 `session_history` 長度恆 = `conversation_history`−1 的 off-by-one）。已補標。教訓：加 early-return gate 到既有多入口路徑時，必 grep 窮舉「所有本該通過此 gate 的合法路徑」逐一確認放行（見 `memory/lessons-frontend.md` 同名教訓）。行號為 2026-07-14 修復後狀態、會漂移，以場景為準。

| 行號 | 場景 | mutate 對象 |
|-----|------|-----------|
| 3321 | Search query submit | `conversationHistory.push(query)` |
| **（search.js 結果完成 callback）** | **一般搜尋結果完成（[R ff989303] 補標）** | **`sessionHistory.push({...結果})` 後——完成才是結果落地點，query submit 只有 query** |
| 3509 | DR query submit | 同上 |
| **（deep-research.js final_result handler）** | **DR final_result 完成（[R ff989303] 補標）** | **push 最終 entry 後——`displayDeepResearchResults` 已 clear dirty，須 re-mark** |
| **（knowledge-graph.js rerun final_result）** | **KG 編輯 rerun 完成（[R ff989303] 補標）** | **rerun entry push 後同理 re-mark** |
...
定義：`AuthManager.USER_SCOPED_KEYS`（`static/js/core/auth-manager.js`，grep `static USER_SCOPED_KEYS`，~:79；v4.0 從 `news-search.js` 遷出）：
...
### 8.3 ~~`_clearUserScopedStorageIfUserChanged(newUserId)`~~ — 已移除（Task 13 cleanup）

> ⚠️ **此 helper 已完全從 codebase 移除**（v4.0 Task 13 cleanup；`static/js/core/auth-manager.js` 內 `// Task 13 cleanup` 註解為證，明言超集 API 為 `UserStateSync.runInitSync` → `fullReset`（= `clearUserScopedState` + `resetMainUI`））。原本「偵測 user 變化才清」的 conditional 語義被 Task 5 把 `login()` 改走 `runInitSync` 後變成 double-clear，遂於 Task 13 刪除。本節保留歷史語義說明，**不保留舊實作 code block**。

**歷史邏輯**：比對 `localStorage.authUser.id` vs 新 `user.id`，identity 變化才 iterate `AuthManager.USER_SCOPED_KEYS` 逐一 `removeItem`；同 user 重登保 cache（D-2026-03-13）。

**現況取代**：
- **Login（trigger A）** / **identity change（trigger B）**：`UserStateSync.runInitSync` 內部先 `fullReset()`（**無條件清**，不再做「同 user 保 cache」的 conditional —— B2B 共用電腦安全 > 重登保 cache，CEO 拍板 D-2026-05-01）→ 再 `fetchInit` + `applyInit` 重新 hydrate。
- 舊的 paired calls（`savedSessions.length = 0` / `renderLeftSidebarSessions()` / `clearUserScopedState()`）現由 `fullReset` + `applyInit` 統一執行，外部無獨立 caller。

### 8.4 `_handleAuthFailure` 也清（CEO 拍板）

實作：`static/js/core/auth-manager.js`（grep `_handleAuthFailure()`，:328-352；v4.0 從 `news-search.js` 遷出）。

呼叫路徑：
- `AuthManager.logout()`：使用者主動登出
- `AuthManager.refreshToken()` catch：refresh failed
- `checkAuthOnLoad()` 401 path：commit `138ae61` 改寫（現於 `core/page-bootstrap.js`）
- `btnLogoutAll` click：登出全部裝置

> ⚠️ **下方 code block 為歷史實作（v4.0 遷出前的 news-search.js 版）**。現況 `_handleAuthFailure` **不再逐步展開這些清理動作**，而是委派給單一 `UserStateSync.fullReset({ keepInviteToken: false })`（`core/state-sync.js`），再收尾 null 掉 `_accessToken`/`_user` + `updateAuthUI()` + `hideMainUI()` + `showAuthModal('login')`。`fullReset` = `clearUserScopedState`（含 `_cancelPendingSave` + USER_SCOPED_KEYS + 6 個 main-UI globals）+ `resetMainUI`。下方步驟的**清理範圍語義仍成立**，但已收斂為單一 `fullReset` 呼叫。權威現況見 §8.5。
...
> **狀態**：D-2026-05-13 之前用來補清「6 個 resetConversation 沒涵蓋的 user-scoped main-UI globals」的 legacy helper 已**完全從 codebase 移除**（Task 13 cleanup，見 `static/js/core/auth-manager.js` 內 `// Task 13 cleanup` 註解 — 該段註解明確指出超集 API 為 `UserStateSync.runInitSync` → `fullReset`（= `clearUserScopedState` + `resetMainUI`））。本節保留高層歷史脈絡，**不保留舊實作的 code block**。
...
**詳細實作**：見 `static/js/core/state-sync.js`（`UserStateSync` module）、`docs/in progress/plans/frontend-init-sync-refactor-plan.md`、`docs/decisions.md` D-2026-05-13。
