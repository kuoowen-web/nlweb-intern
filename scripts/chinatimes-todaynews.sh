#!/bin/bash
# 桌機版 Chinatimes todaynews.xml 每日收集腳本
# 每天抓取 todaynews.xml 中的文章（最近 2-3 天、約 1000 篇）
# retry_urls mode 會自動 skip 已爬過的，重複執行安全
#
# 使用方式（Git Bash cron 或手動執行）：
#   bash /c/users/user/nlweb/scripts/chinatimes-todaynews.sh
...
curl -s -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" -o "$SITEMAP_FILE" "$SITEMAP_URL"
...
print(f"[todaynews] Request body written ({len(urls)} URLs)")
PYEOF
